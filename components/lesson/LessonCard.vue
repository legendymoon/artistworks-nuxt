<template>
  ...
  >
    <div :class="view === 'list' ? 'flex gap-4' : 'min-h-[190px] sm:min-h-0'">
        <div
          :class="view !== 'list' ? 'h-32 w-full' : 'mt-3 ml-4 h-20 w-32'"
          class="relative"
        >
          <img
            :src="lesson.thumbnail"
            alt="Thumbnail"
            class="rounded-[5px] object-cover w-full h-full"
          />
          <div
            v-if="lesson.progress >= 100"
            class="absolute inset-0 bg-black bg-opacity-70 z-10 rounded-[5px]"
          ></div>
          <div
            v-if="lesson.progress >= 100"
